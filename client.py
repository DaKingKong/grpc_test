...
RATE = 8000  # Increased from 16000 to CD quality
CHUNK = 4096  # Larger chunk size for better quality
...
def generate_stream_events():
    ...
    dialog_init = ringcx_streaming_pb2.DialogInitEvent(
        account=ringcx_streaming_pb2.Account(id="test_account"),
        dialog=ringcx_streaming_pb2.Dialog(id="test_dialog", type=ringcx_streaming_pb2.DialogType.INBOUND)
    )
    yield ringcx_streaming_pb2.StreamEvent(
        session_id=session_id,
        dialog_init=dialog_init
    )
    
    # Then send a segment start event
    segment_start = ringcx_streaming_pb2.SegmentStartEvent(
        segment_id=segment_id,
        participant=ringcx_streaming_pb2.Participant(id="user", type=ringcx_streaming_pb2.ParticipantType.CONTACT),
        audio_format=ringcx_streaming_pb2.AudioFormat(
            codec=ringcx_streaming_pb2.Codec.L16,
            rate=RATE,
    ...
    try:
        while running:
            ...
            audio_content = ringcx_streaming_pb2.AudioContent(
                payload=audio_chunk,
                seq=seq,
            ...
            segment_media = ringcx_streaming_pb2.SegmentMediaEvent(
                segment_id=segment_id,
        ...
        segment_stop = ringcx_streaming_pb2.SegmentStopEvent(
            segment_id=segment_id
        )
        yield ringcx_streaming_pb2.StreamEvent(
            session_id=session_id,
...
def run():
    ...
    try:
        ...
        # with grpc.insecure_channel('localhost:10080') as channel:
            # Create stub
            stub = ringcx_streaming_pb2_grpc.StreamingStub(channel)
            
            print("Connecting to server...")
            
            # Call Stream method with the generator and metadata
            response = stub.Stream(generate_stream_events(), metadata=metadata)
